...
// U8g2_for_Adafruit_GFX ライブラリのインクルード
#include <U8g2_for_Adafruit_GFX.h>
// U8g2_for_Adafruit_GFX に含まれる日本語フォントのインクルード
// ここでは M+ 1C Regular 14pt を使用します。
// 他のサイズや種類（例: u8g2_font_mplus_12_proportional, u8g2_font_notosansjp_bold15）も選択可能です。
#include <Fonts/u8g2_font_mplus_14_proportional.h> // 日本語対応フォント
...
void displayMessage(const char* title, const char* message) {
  ...
  do {
    ...
    // 日本語フォントを使用
    display.setFont(&u8g2_font_mplus_14_proportional);
    display.setCursor(5, 30);
    display.print(title); // タイトルは日本語も考慮

    display.setFont(&u8g2_font_mplus_14_proportional); // 日本語フォントを使用
...
void updateDisplay() {
  ...
  do {
    ...
    char currentTimeStr[30]; // バッファサイズを拡張
    ...
    // 発車時刻を大きく表示（英数字のみなので既存フォントも利用可能ですが、ここでは統一性を保つため日本語フォントと同じサイズを使用）
    display.setFont(&FreeMonoBold18pt7b); // 時刻は英数字なので既存フォントでOK
    ...
    // 文字数制限（電子ペーパーの幅に合わせて調整）
    // 日本語フォントの場合、半角文字数での制限は不適切なので、ピクセル幅を考慮するか、表示可能文字数を経験的に設定
    // 1.54inchの場合、14ptの日本語フォントで全角7-8文字程度が目安
    if (destInfo.length() > 16) { // 英数字換算での制限。日本語の場合は文字数が少なくなる。
      destInfo = destInfo.substring(0, 16); // 日本語の途中で切れる可能性あり
      // 厳密には display.getTextBounds() などでピクセル幅を計算して調整すべき
    }
    ...
    char nextNextStr[35]; // バッファサイズを拡張
    ...
    if (nextNextDisplay.length() > 20) { // 英数字換算での制限。日本語の場合は文字数が少なくなる。
      nextNextDisplay = nextNextDisplay.substring(0, 20); // 日本語の途中で切れる可能性あり
    }
